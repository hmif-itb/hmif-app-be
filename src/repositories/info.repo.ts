...
export async function GetListInfosSearchCategoryUnread(db: Database, userId: string, search: string, category: string) {
    const getReadInfosByUser = db.select({infoId : userReadInfos.infoId}).from(userReadInfos).where(
        and(
...
export async function GetListInfosCategoryUnread(db: Database, userId: string, category: string) {
    const getReadInfosByUser = db.select({infoId : userReadInfos.infoId}).from(userReadInfos).where(
        and(
            eq(userReadInfos.userId, userId)
        )
